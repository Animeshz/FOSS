...
* FOSS contribution in general showcases your work as its open and free to look at.
  <!--(F for free to run study share improve)-->
...
```mermaid
flowchart LR
  A[<u>Source Code</u><br>*.java / *.kt / *.py] --> B([<u>Compiler</u><br>javac / kotlinc]) --> C[<u>ByteCode</u><br>*.class] --> F([<u>Interpreter</u><br>java])
  D([Programmer]) --> A
  F --> E([Computer])
```
